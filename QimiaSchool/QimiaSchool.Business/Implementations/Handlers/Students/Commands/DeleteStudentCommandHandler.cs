...
namespace QimiaSchool.Business.Implementations.Handlers.Students.Commands
{
    public class DeleteStudentCommandHandler : IRequestHandler<DeleteStudentCommand, Unit>
    {
        ...
        public async Task<Unit> Handle(DeleteStudentCommand studentCommand, CancellationToken cancellationToken)
        {
            await _studentManager.DeleteStudentByIdAsync(studentCommand.Id, cancellationToken);
            return Unit.Value;
        }
    }
